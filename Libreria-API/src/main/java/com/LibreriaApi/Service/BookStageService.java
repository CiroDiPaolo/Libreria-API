...
public class BookStageService {
    ...
    public void updateBookStageService(BookStage bookStage) {
        Optional<BookStage> existing = bookStageRepository.findById(bookStage.getIdBookStage());
        if (existing.isPresent()) {
            bookStageRepository.save(bookStage);
        } else {
            throw new RuntimeException("No se encontró un estado para el id: " + bookStage.getIdBookStage());
        }
    }
